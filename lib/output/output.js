		...
		fs.readFile(config_url, 'utf8' ,function(err, data){
					...
					for(var i=0;i<configuration.graphTypes.length;i++){
						for(var j=0;j<configuration.graphTypes[i].series.length;j++){
							if(configuration.graphTypes[i] == "image"){
								configuration.graphTypes[i].url = returned_run.user + "/" + returned_run.step + "/" + returned_run.stepID + "/" + configuration.graphTypes[i].url;
			...
			function readAndConfigure(step){
				...
				fs.readFile(config_url, 'utf8' ,function(err, data){
					...
					for(var i=0;i<stepConfig.graphTypes.length;i++){
						if(stepConfig.graphTypes[i].graphClass == "image"){
								stepConfig.graphTypes[i].url = step.user + "/" + step.step + "/" + step.stepID + "/" + stepConfig.graphTypes[i].url;
						} else {
