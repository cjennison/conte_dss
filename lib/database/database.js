...
function getRuns(stepname, basin_id, username, cb){
	var runsToRead = [];
	var that = this;
	...
	var collection = new mongodb.Collection(db_client, 'users');
	collection.find({"user":username, "status":"COMPLETED"}).toArray(function(err, docs){
			//console.log(docs);
			...
			for(var i=0;i<dbruns.length;i++){
				if(dbruns[i].basinid == basin_id && dbruns[i].step == stepname){
					runsToRead.push(username + "/" + stepname + "/" + dbruns[i].stepID + "/settings.json")
				}
			}
